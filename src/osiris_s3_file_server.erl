-module(osiris_s3_file_server).
-behaviour(gen_server).

%% External exports
-export([format_error/1]).
-export([start/0, start_link/0, stop/0, open/2]).

%% gen_server callbacks
-export([init/1, handle_call/3, handle_cast/2, handle_info/2,
         terminate/2, code_change/3]).

-define(FILE_SERVER, ?MODULE).      % Registered name
-define(FILE_IO_SERVER, osiris_s3_io_server).  % Module
