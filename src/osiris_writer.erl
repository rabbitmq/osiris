...
-record(?MODULE,
        {cfg :: #cfg{},
         ...
         committed_chunk_id = -1 :: osiris:offset()}).
  ...
  when ?IS_STRING(Name) ->
    ...
    {LastChunkId, NextOffset} =
        case osiris_log:tail_info(Log) of
            ?TAIL_INFO_NEXT(TailNextOffs, TailChId) when Replicas == [] ->
...
handle_batch(Commands,
    ...
    case catch lists:foldr(fun handle_command/2,
        ...
        {#?MODULE{log = Log0} = State1, Entries, Replies, Corrs, Trk1, Dupes} ->
            ...
            ReplicaTI = maps:values(State2#?MODULE.replica_state),
            {NOffs, {_, ChkId, _}} = agreed_commit([TailInfo | ReplicaTI]),
            ...
            State = case ChkId > ChkId0 of
                        true ->
                            ...
                            State2#?MODULE{committed_chunk_id = ChkId,
                                           duplicates = RemDupes,
...
handle_command({cast, {ack, ReplicaNode, TailInfoOrOffsTs}},
    ...
    {Offset, TI} = case TailInfoOrOffsTs of
                       ?TAIL_INFO(ChkId) ->
                           {ChkId, TailInfoOrOffsTs};
                       {Offs, Ts} ->
                           {Offs, {0, {0, Offs, Ts}}}
                   end,
    ReplicaState =
        case ReplicaState0 of
            #{ReplicaNode := ?TAIL_INFO(O)} when Offset > O ->
...
handle_command({call, From, query_replication_state},
               {#?MODULE{log = Log,
                ...
                Corrs, Trk, Dupes}) ->
    Result0 = maps:map(fun(_, ?TAIL_INFO(O, T)) ->
                               {O, T}
                       end, R),
    %% need to merge pending tracking entries before read
...
-spec agreed_commit([osiris:tail_info()]) -> osiris:tail_info().
agreed_commit(Indexes) ->
    SortedIdxs = lists:sort(fun(?TAIL_INFO(ChkId0), ?TAIL_INFO(ChkId1)) ->
                                    ChkId0 > ChkId1
                            end, Indexes),
