...
init_replica(LeaderOverview,
             #{name := Name,
               epoch := Epoch,
               leader_pid := LeaderPid,
               reference := ExtRef} = Config) ->
    Node = node(LeaderPid),
    {Min, Max} = application:get_env(osiris, port_range, ?DEFAULT_PORT_RANGE),
    ...
    case osiris_replica_reader:start(Node, ReplicaReaderConf) of
        {ok, RRPid} ->
            ...
            ManifestMod = application:get_env(osiris, log_manifest,
                                              osiris_log),
            osiris_util:cache_reader_context(self(), Dir, Name, Shared,
                                             ExtRef, ManifestMod,
                                             fun(Inc) ->
