...
do_start(Spawn, Owner, _FileName, ModeList) ->
    Self = self(),
    Ref = make_ref(),
          ...
          fun() ->
                  ...
                  case parse_options(ModeList) of
                      {ReadMode, UnicodeMode, Opts0} ->
                          _Opts = maybe_add_read_ahead(ReadMode, Opts0),
...
server_loop(#state{mref = Mref} = State) ->
    receive
        ...
        {io_request, From, ReplyAs, _Request} when is_pid(From) ->
            Reply = not_yet_implemented,
