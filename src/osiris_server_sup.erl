...
init([#{name := Name} = Config]) ->
    ...
    Crc = #{id => crc_name(Name),
            start =>
                {osiris_competing_read_coordinator, start_link,
                 [Config, self()]},
            restart => temporary,
            shutdown => 5000,
            type => worker},
...
is_children(#{name := Name, leader_node := Leader}) ->
    case supervisor:get_childspec({?SUP, Leader}, supervisor_name(Name))
    of
        {ok, _} ->
            true;
        _ ->
...
get_child(SupPid, Name) when is_pid(SupPid) ->
    case [Pid
          || {Id, Pid, _, _} <- supervisor:which_children(SupPid), Id == Name]
    of
        [] ->
            {error, not_found};
        [Pid] ->
...
get_child(SupName, Name) ->
    case [Pid
          || {Id, Pid, _, _}
                 <- supervisor:which_children(osiris_server_sup_sup),
             Id == SupName]
    of
        [] ->
            {error, not_found};
...
supervisor_name(Name) ->
    lists:flatten(
        io_lib:format("~s_supervisor", [Name])).
